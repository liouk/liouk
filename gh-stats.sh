...
num_reviews=$(gh search prs --assignee liouk --visibility public --json id | jq '. | length')
...
sed -i "s/label=Reviews\&message=[[:digit:]]\+/label=Reviews\&message=$num_reviews/g" README.md
